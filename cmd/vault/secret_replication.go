...
import (
	...
	"github.com/soerenschneider/sc-agent/internal/services/components/vault_secrets_replication"
	"github.com/soerenschneider/sc-agent/internal/services/components/vault_secrets_replication/formatter"
)
...
func BuildSecretReplication(conf *vault.SecretsReplication) (*vault_secrets_replication.SecretReplicationService, error) {
	if conf == nil {
	...
	kv2Client, err := vault_secrets_replication.NewClient(client.Client().KVv2(conf.Kv2Mount))
	if err != nil {
		return nil, err
	}
	...
	return vault_secrets_replication.NewService(kv2Client, syncRequests)
}

func buildSyncSecretRequests(conf vault.SecretsReplication) ([]domain.SecretReplicationItem, error) {
	ret := []domain.SecretReplicationItem{}

	for _, req := range conf.ReplicationRequests {
