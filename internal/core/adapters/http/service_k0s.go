...
func (s *HttpServer) K0sPostAction(w http.ResponseWriter, r *http.Request, action K0sPostActionParamsAction) {
	...
	var err error
	if action == K0sPostActionParamsActionStart {
		err = s.services.K0s.Start()
	} else if action == K0sPostActionParamsActionStop {
