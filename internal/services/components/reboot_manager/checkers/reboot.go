...
func (r *RebootCheckerDnf) IsHealthy(ctx context.Context) (bool, error) {
	...
	if err != nil {
		...
		if errors.As(err, &exitError) {
			...
			case 1:
				log.Info().Str("component", "reboot-manager").Str("component", "reboot-manager").Str("checker", "reboot-dnf").Msg(stdout.String())
				return false, nil
			default:
				log.Warn().Str("component", "reboot-manager").Int("exitcode", exitError.ExitCode()).Msg("got unknown exitcode from running dnf needs-restarting")
			}
		}
	}
