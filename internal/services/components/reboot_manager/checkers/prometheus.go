...
func (c *PrometheusChecker) LoadTlsClientCerts(info *tls.CertificateRequestInfo) (*tls.Certificate, error) {
	...
	if err != nil {
		log.Error().Str("component", "reboot-manager").Err(err).Msg("user-defined client certificates could not be loaded")
	}
...
func (c *PrometheusChecker) query(ctx context.Context, name, query string) (bool, error) {
	...
	if len(warnings) > 0 {
		log.Warn().Str("component", "reboot-manager").Str("checker", "prometheus").Msgf("warning for query '%s': %v", name, warnings)
	}
...
func (z *ZerologAdapter) Debug(msg string, keysAndValues ...interface{}) {
	log.Debug().Str("component", "reboot-manager").Str("checker", "prometheus").Interface("details", keysAndValues).Msg(msg)
}

// Info logs an info-level message
func (z *ZerologAdapter) Info(msg string, keysAndValues ...interface{}) {
	log.Info().Str("component", "reboot-manager").Str("checker", "prometheus").Interface("details", keysAndValues).Msg(msg)
}

// Warn logs a warning-level message
func (z *ZerologAdapter) Warn(msg string, keysAndValues ...interface{}) {
	log.Warn().Str("component", "reboot-manager").Str("checker", "prometheus").Interface("details", keysAndValues).Msg(msg)
}

// Error logs an error-level message
func (z *ZerologAdapter) Error(msg string, keysAndValues ...interface{}) {
	log.Error().Str("component", "reboot-manager").Str("checker", "prometheus").Interface("details", keysAndValues).Msg(msg)
}
