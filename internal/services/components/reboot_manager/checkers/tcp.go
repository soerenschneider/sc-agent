...
func (c *TcpChecker) IsHealthy(ctx context.Context) (bool, error) {
	...
	if err == nil && conn != nil {
		defer conn.Close()
		log.Debug().Str("component", "reboot-manager").Str("checker", "tcp").Msgf("Connecting to %s succeeded", c.Name())
		return true, nil
	}

	if errors.Is(err, syscall.ECONNREFUSED) {
		// receiving this error means the remote system replied
		log.Warn().Str("component", "reboot-manager").Str("checker", "tcp").Err(err).Msgf("Review configuration, connection refused to %s", c.Name())
		return true, nil
	}

	log.Error().Str("component", "reboot-manager").Str("checker", "tcp").Err(err).Msgf("Connectivity checker '%s' encountered errors", c.Name())
	return false, nil
}
