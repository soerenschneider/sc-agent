...
func (n *NeedrestartChecker) detectUpdates(out string) (bool, bool) {
	...
	if len(matches) >= 2 {
		...
		if err != nil {
			log.Error().Str("component", "reboot-manager").Str("checker", "needrestart").Msgf("could not parse 'NEEDRESTART-KSTA': %v", err)
		} else if val > n.rebootMinKsta {
			kernelUpdate = true
			log.Info().Str("component", "reboot-manager").Str("checker", "needrestart").Int("KSTA", val).Msg("Kernel updates detected")
		}
	} else {
		log.Warn().Str("component", "reboot-manager").Str("checker", "needrestart").Msg("Could not find KSTA information")
	}

	// check for service upgrades
	if strings.Contains(out, "NEEDRESTART-SVC:") {
		svcUpdates = true
		log.Info().Str("component", "reboot-manager").Str("checker", "needrestart").Msg("Service updates detected")
	}
