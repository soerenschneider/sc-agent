...
func (r *StateCheckerAnd) CheckAgent(agent state.Agent) bool {
	...
	for wantedType, wantedFor := range r.wants {
		log.Debug().Str("component", "reboot-manager").Str("checker", StateCheckerAndName).Str("agent", agent.CheckerNiceName()).Msgf("wanted=%s, wantedFor=%v", wantedType, wantedFor)
		if currentState == wantedType {
			log.Debug().Str("component", "reboot-manager").Str("checker", StateCheckerAndName).Str("agent", agent.CheckerNiceName()).Msgf("currentState=%s, duration=%v", currentState, agent.GetStateDuration())
